    ...
    <div class="left-align-wrapper events-list">
        <div class="page-intro">
            <div class="page-intro-inner">
                ...
                <p>{!! nl2br($user->bio) !!}</p>
                @if (Auth::check())
                        <a class="link-effect edit-user-details" href="{{ route('{city}.users.edit', ['city' => Request::route()->getParameter('city'), 'users' => $user->slug])}}">Edit your details</a>
                @endif
            </div>
        </div>
        ...
        @else
            @foreach( $events as $ev )
                ...
                <li id="event-item-{{ $ev->id }}" class="event clear {{ ($event && $event->id === $ev->id) ? 'event--active' : '' }}">
                    <a href="{{ route('{city}.events.show', array('nyc', $ev->slug)) }}">
